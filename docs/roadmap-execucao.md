...
| **F1** | 🔶 **Streamlit moderno — parcial (02/08/2026).** ✅ `renderizar_grid()` com AgGrid · `tratar_erros()` · `st.navigation` nativa filtrada por permissão · `st.dialog` e grid em Fornecedores e Compras · grid em Pacientes, Médicos, Procedimentos, Unidades, Setores e Estoque · smoke das 28 telas. ⬜ **Falta:** `admin_usuarios.py` (ainda com `<div>` sobre `st.columns`), `st.dialog` em glosas/contas/guias, `st.fragment` nas telas de estado por linha | — | F0 | F2, F3 | evolucao §2 |
