...
| **F1** | **Streamlit moderno** — `st.navigation`; componente `renderizar_grid`; `st.fragment`/`st.dialog` nas 6 telas de estado por linha; `tratar_erros()`; grids de Fornecedores e Compras | — | F0 | F2, F3 | evolucao §2 |
| **F2** | **BI onda 1** — 8 bugs + 2 problemas de grão; modelo dimensional; ETL idempotente; camada de métricas; Altair; filtro de período; 4 dashboards; testes | `0014` | F0 | F1, F3 | **[plano-bi.md](plano-bi.md) §10** |
